...
static void printList(const std::list<couple> & l) {
  ...
  while(!copy.empty()) {
    std::cout << copy.back().first << " ; " << (char)copy.back().second << std::endl;
    copy.pop_back();
  }
  std::cout << std::endl;
}
...
unsigned short int Game::score(const Board * b, const int & pos,
  ...
  if (direction == 'H') {
    ...
    i -= 1;
    std::cout << "parcours bonus" << std::endl;
...
unsigned short int Game::score (const Move & m) const {
  ...
  if (m.direction == 'H') {               //On joue vers le HAUT
    ...
    while ( word_pos >= 0
      ...
      std::cout << (int)board_pos << std::endl;
      ...
      if(direction == 'H'){ // Horizontalement
        ...
        if (((x + 1 <= 14)
            ...
            ||(x + 1 > 14)){
              ...
              //          << " de " << new_move.first_square
              //          << " avec j1  : " <<static_cast<int>(j1)
              //          << " et j2  : " <<static_cast<int>(j2)
              //          << " ; qui donne : " << std::flush;
